...
class urlfetcher {
	...
	public function post($url, $data, $content_type = "text/html; charset=utf-8", $header='', $conn_timeout = false, $timeout = false){
		...
		if (!$conn_timeout) $conn_timeout = $this->conn_timeout;
		if (!$timeout) $timeout = $this->timeout;
		if(is_object($pdl)) { $this->pdl->log('urlfetcher', 'post url: '.$url.' method: '.$this->method); }
	...
	private function post_curl($url, $data, $content_type, $header, $conn_timeout, $timeout){
		...
		$arrCurlInfo = curl_getinfo($curl);
		$curl_error = curl_errno($curl);
		if(is_object($pdl)) { $this->pdl->log('urlfetcher', 'Curl Error Nr. '.$curl_error); }
		if(is_object($pdl)) { $this->pdl->log('urlfetcher', 'Curl Info: '.print_r($curl_error, true)); }

		curl_close($curl);

		if(is_object($pdl)) { $this->pdl->log('urlfetcher', 'Response Code: '.$code); }
		if(is_object($pdl)) { $this->pdl->log('urlfetcher', 'Response: '.strlen($getdata).'; First 200 Chars: '.htmlspecialchars(substr($getdata, 0, 200))); }
		return trim($getdata);
	}
